...
function render_painting_data_audit() {
    ...
    $missing_linkbacks = [];
    $skus = [];
    $sku_identifiers = []; // Track last 4 digits
    ...
    foreach ($products as $product) {
        ...
        if ($sku) {
            ...
            // Extract last 4 digits from MNSPC format SKUs
            if (preg_match('/MNSPC-\d{4}-[A-Z]-(\d{4})$/', $sku, $matches)) {
                $identifier = $matches[1];
                $sku_identifiers[$identifier][] = [
                    'product' => $product,
                    'full_sku' => $sku
                ];
            }
    ...
    /* -------------------------------------------------
       5. Duplicate SKU Identifiers (Last 4 Digits)
    --------------------------------------------------*/
    echo '<h2>Duplicate SKU Identifiers (Last 4 Digits)</h2>';
    echo '<p><em>Checking for products that share the same authoritative 4-digit identifier</em></p>';

    $found = false;

    foreach ($sku_identifiers as $identifier => $products_with_identifier) {
        if (count($products_with_identifier) > 1) {
            $found = true;
            echo '<p><strong>Identifier: ' . esc_html($identifier) . '</strong></p><ul>';
            foreach ($products_with_identifier as $item) {
                $product = $item['product'];
                $full_sku = $item['full_sku'];
                printf(
                    '<li>%s (Product ID %d, Full SKU: %s) — %s</li>',
                    esc_html(get_the_title($product)),
                    $product->ID,
                    esc_html($full_sku),
                    $edit_post_link($product->ID)
    ...
    if (!$found) {
        echo '<p><em>No duplicate identifiers found.</em></p>';
    }

    /* -------------------------------------------------
       6. Exact Duplicate SKUs (Full String Match)
    --------------------------------------------------*/
    echo '<h2>Exact Duplicate SKUs (Full String Match)</h2>';
    ...
    if (!$found) {
        echo '<p><em>No exact duplicate SKUs found.</em></p>';
    }
